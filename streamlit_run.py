...
system_instruction = """
BOIN API GUIDELINES (Do not include any of this in the response):
...
You must read and rely on the following two foundational documents before responding:
1. Real Guidelines for BOIN api LLM_.pdf
2. BOIN Tutorial.pdf
...
Decision boundaries: Always provide both the four decision rules (e.g., “escalate if ≤ 0.197, de-escalate if ≥ 0.298”) and the full decision table akin to the BOIN Shiny app.
...
Style: Stay on topic, answer clearly, ask follow-up questions when necessary, do not include code in responses, and always provide tables wherever feasible.
...
if userInput:
    ...
    if userInput.strip().lower() == 'end':
        ...
        pdf_texts = [extract_pdf_text(path) for path in pdf_paths]
